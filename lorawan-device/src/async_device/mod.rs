...
pub struct Device<I, R, C, T, G, const N: usize = 256, const D: usize = 1>
where
    I: DeviceHandler,
...
impl<I, R, C, T, const N: usize> Device<I, R, C, T, rng::Prng, N>
where
    I: DeviceHandler,
    R: radio::PhyRxTx + Timings,
    C: CryptoFactory + Default,
...
impl<I, R, C, T, G, const N: usize, const D: usize> Device<I, R, C, T, G, N, D>
where
    I: DeviceHandler,
    R: radio::PhyRxTx + Timings,
    C: CryptoFactory + Default,
    ...
    G: RngCore,
{
    ...
    async fn handle_mac_response(
        ...
        match response {
            ...
            #[cfg(feature = "certification")]
            mac::Response::DeviceHandler(event) => match event {
                mac::DeviceEvent::ResetMac => {
                    device.reset_mac();
                    Ok(None)
                }
                mac::DeviceEvent::ResetDevice => {
                    device.reset_device();
                    Ok(None)
                }
                mac::DeviceEvent::TxPeriodicityChange { periodicity } => {
