...
fn main() {
    let crate_name = env::var("CARGO_PKG_NAME").unwrap().to_ascii_uppercase().replace('-', "_");
    ...
    for (name, default) in CONFIGS {
        configs
            .insert(*name, ConfigState { value: *default, seen_env: false, seen_feature: false });
    }

    let prefix = format!("{crate_name}_");
    for (var, value) in env::vars() {
        if let Some(name) = var.strip_prefix(&prefix) {
            let Some(cfg) = configs.get_mut(name) else { panic!("Unknown env var {name}") };
